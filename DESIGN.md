...
- **Shade (waiting / context)** — the **striped shade pin**: cream base
  (`--text`) with Jordy Blue (`--blue-300`, `#9CBDE7`) diagonal stripes at
  **135°**, 5.5px period, 50% fill — the logo's sunglasses-lens motif. Solid
  Jordy border; dot/heart read solid Delft (`--bg`); time is 700 Delft ink on a
  cream halo so the stripes show through while staying legible. (Reworks the old
  flat Jordy fill into the striped lens — dialed in `pill-lab.html`.)
...
**Weather glyphs** (`_wxSvg*` in `weather.js`: sun / cloud-sun / cloudy / cloud / cloud-rain; plus the `closed` moon in `ui-plan-preview.js`) are genuine Lucide outline at the standard `stroke-width: 2`. Because they sit over the map / FTS (any background colour), they carry a legibility casing: `filter: var(--icon-casing)` — a tight 360° dark halo (via the `.wx-sky-icon` class). Casings are sanctioned ("legibility scrims … are not 'fills'"); the casing is dropped (`filter: none`) on the light top-strip, where the glyph goes ink.
