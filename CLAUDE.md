...
| `045-sun-alert-tables.sql` | `venue_sun_windows` (raw geometric windows per venue per date, pre-baked by `scripts/precompute-sun-windows.mjs`) + `weather_oslo` (hourly forecast snapshot for Oslo). Both deny-all RLS — only service-role writes via the §5a edge functions. `wx_bucket(timestamptz)` and `oslo_hour_bucket(date, hour)` helpers for sql/047. |
| `046-sun-alert-cron-schedules.sql` | `_call_edge(url_key)` helper + pg_cron schedules: `fetch-weather` every 30 min, `sync-sun-windows` daily at 03:30 UTC. Both edge functions gate on the shared `PUSH_TRIGGER_SECRET` from `_app_settings`. |
| `047-process-sun-alerts.sql` | `process_sun_alerts()` pg_cron every 5 min — joins `sun_alerts × venue_sun_windows(today) × weather_oslo(window-start hour)`. For each enabled subscription whose venue has an upcoming raw window within `notify_minutes_before` AND weather-clear at the start, writes the bell row + fires push via `_do_send_push`. Replaces the client `_evalSunAlerts` 60s loop. `sun_alerts.venue_name` denormalized so the push body doesn't need a join to venues.json. Bell row category is `'weather'` (matches `notifications_category_check`); the toast stays `'alert'` on the client. |
| `048-hybrid-payload-phase-2.sql` | Extends the hybrid `lead.bodyKey` + `lead.bodyVars` payload to the remaining four `notif_on_*` bell-writing triggers (`friendship_accepted`, `plan_invite_created`, `plan_invite_response`, `plan_cancelled`). Bell + toast render in the user's locale via `_bellBodyFromDescriptor`; `{when}` is synthesized client-side from `nav.plannedAt` via `_renderPlannedWhen` so date/time stays locale-aware without baking `notif_when_label` into the structured payload. Push body stays NO. |
| `049-process-weather-no-sun-alerts.sql` | `process_weather_no_sun_alerts()` pg_cron every 30 min — checks city-wide `venue_sun_windows`; if no venue has a window ending after now-hour today, writes a `weather_no_sun:<date>` bell row for every active user with `notif_prefs.weather` not false. Bell-only (no push, matches the prior client `bellOnly: true`). Replaces the client `_evalNoSunToday`. Also adds the missing `user_preferences.notif_prefs` JSONB column (the client write at `notifications.js` was silently failing). |
| `050-process-weather-best-sun-alerts.sql` | `process_weather_best_sun_alerts()` pg_cron every 30 min — finds the hour with most sunny venues in [now, 21] from `venue_sun_windows`; if peak ≥ 15 venues AND weather-clear AND meaningfully better than now (1.3× count or +5), expands the peak block at 70% threshold and writes a `weather_best_sun:<date>` bell row for every active user with weather notifs enabled. Bell-only, once per user per day. Replaces the client `_evalBestSunWindow`. Internal 18:00 Oslo guard skips late-day ticks. |
| `051-friend-request-bell-row.sql` | `notify_friend_request` extended to ALSO write a `friend_request:<friendship_id>` bell row (hybrid `bodyKey`/`bodyVars`) alongside its existing push. Realtime delivers, `_bellRowToToast` (auth.js) surfaces a per-request toast with inline Accept via `nav.friendshipId`. Replaces the client `_evalIncomingFriendRequest`. Push body also brought to NO for consistency with the other social pushes. |
