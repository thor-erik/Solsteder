...
| `047-process-sun-alerts.sql` | `process_sun_alerts()` pg_cron every 5 min — joins `sun_alerts × venue_sun_windows(today) × weather_oslo(window-start hour)`. For each enabled subscription whose venue has an upcoming raw window within `notify_minutes_before` AND weather-clear at the start, writes the bell row + fires push via `_do_send_push`. Replaces the client `_evalSunAlerts` 60s loop. `sun_alerts.venue_name` denormalized so the push body doesn't need a join to venues.json. |
